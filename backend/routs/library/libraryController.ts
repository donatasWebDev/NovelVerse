...
export const getBookPage = async (req: Request, res: Response) => {
  let skip = parseInt(req.query.page as string)-1 || 0;
  const limit = 20;
  skip = skip * limit;
  try {
    const books = await prisma.book.findMany({skip: skip, take: limit})
...
export const bookAddChapters = async (req: Request, res: Response) => { // poulkates Chapters to a Book
  try {
    ...
    if (!req.body.nr) {
      res
      .status(404)
      .json({ message: 'requset Body doe not have chapters nr' });
      return 
    }
...
export const getChapter = async (req: Request, res: Response) => {  //!BROKEN FIX CODE 
  try {
    ...
    if (!req.body.nr || !req.body.bookURL) {
      res.status(404).json({ message: 'Book URL or Chapter number not provided' });
      return;
    }
